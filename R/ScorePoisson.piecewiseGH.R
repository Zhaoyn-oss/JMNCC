ScorePoisson.piecewisefJM <- function(thetas){
  ...
  for(i in 1:ncz){ 
    rs <- Z[, i]*(y-mu.y) 
    rs[is.na(rs)] <- 0
    rs_long <- rowsum(rs, id, reorder = FALSE)  
    V[i, ] <- V[i, ] - colSums(rs_long * p.byt, na.rm = TRUE)
     
    haz_i <- 0
    for (k in seq_len(nRisks)) haz_i <- haz_i + (betas[k] * Ztime[, i]) * (delta == k)    
    haz_i_mat <- matrix(rep(haz_i, M), n, M)
    V[i, ] <- V[i, ] - colSums(haz_i_mat * p.byt, na.rm = TRUE)
    ...
    for (k in seq_len(nRisks)) {
      ...
      surv_i_mat <- surv_i_mat + (exp.eta.tw[[k]] * tmp) 
    }
    V[i, ] <- V[i, ] + colSums(surv_i_mat * p.byt, na.rm = TRUE)
  }
  dL <- sqrt(2) * (V %*% tao) 
...
ScorePoisson.piecewisewJM <- function(thetas){
  thetas <- relist(thetas, skeleton = list.thetas)
  betas <- thetas$betas 
  gammas <- thetas$gammas
  alphas <- thetas$alphas 
  ...
  sc2 <- colSums(ki.haz*weight, na.rm = TRUE)
