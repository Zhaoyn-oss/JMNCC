#' wJM-NCC: Likelihood inference with inverse probability weighting approach
#'
#' The wJM-NCC method constructs an inverse probability weighting likelihood function using only data from
#' the NCC sub-cohort, accounting for the potential selection bias in the NCC sampling. The longitudinal sub-model must be a
...
#' @param weightVar Character string for subject-level weights column in `interFact$data`.
...
#' cc.surdata.long <- JM::crLong(data = sur_cc_linear, statusVar="event",
#' censLevel = 1, nameStrata = "CR", nameStatus = "comp.event")
#'
#' # Fit longitudinal model
#' obsObject.cc <- lme4::lmer(obs.value~sample.time+(1+sample.time|subject), data=obs_cc_linear)
#'
#' # Fit Cox model
#' survObject.cc <- survival::coxph(Surv(sur.time, comp.event) ~ gender+strata(CR),
...
#'
#'  obsObject.cc <- lme4::glmer(obs.value~sample.time+(1+sample.time|subject),
#'  data=obs_cc_poisson, family=poisson(link = "log"))
#'  cc.surdata.long <- JM::crLong(data = sur_cc_poisson, statusVar="event",
#'   censLevel = 1, nameStrata = "CR", nameStatus = "comp.event")
#'  survObject.cc <- survival::coxph(Surv(sur.time, comp.event) ~ gender+strata(CR),
#'  data = cc.surdata.long, x = TRUE)
#'  res.wjm <- wJM.NCC(obsObject=obsObject.cc, survObject=survObject.cc,
...
#' }
#' @export
wJM.NCC <- function(obsObject, survObject, timeVar, deltaVar, weightVar,
  ...
  if (length(weightVar) != 1 || !is.character(weightVar))
    stop("\n'weightVar' must be a character string.")

  if (!weightVar %in% names(interFact$data))
    stop("\n'weightVar' does not correspond to one of the columns in the 'interFact:data'.")
  ...
  weight <- interFact$data[[weightVar]]
  weight <- weight[!duplicated(idT)]
  weight <- as.numeric(weight)     # weights for each subject
  ...
  out <- piecewisewJM.fit(obsdata=obsdata, surdata = surdata, id=id,
