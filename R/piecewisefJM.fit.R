piecewisefJM.fit <- function(obsdata, surdata, id, initial.values, control){
  ...
  environment(LogLikGaussian.piecewisefJM) <- environment(ScoreGaussian.piecewisefJM) <- environment(ScoreGaussianMat.piecewisefJM) <- environment()
  environment(LogLikPoisson.piecewisefJM) <- environment(ScorePoisson.piecewisefJM) <-  environment(ScorePoissonMat.piecewisefJM) <- environment()
  ...
  if(fam=="gaussian"){
    out <- optim(thetas, LogLikGaussian.piecewisefJM, ScoreGaussian.piecewisefJM, method = "BFGS", hessian = TRUE,
    ...
    score <- ScoreGaussian.piecewisefJM(unlist(thetas))
    score.mat <- ScoreGaussianMat.piecewisefJM(unlist(thetas)) 
  } 
  if(fam=="poisson"){
    out <- optim(thetas, LogLikPoisson.piecewisefJM, ScorePoisson.piecewisefJM, method = "BFGS", hessian = TRUE,
                 control = list(maxit = control$iter.qN, parscale = parscale, trace = 10 * control$verbose))
    ...
    xi <- thetas$log.xi
    conv <- out$convergence
    score <- ScorePoisson.piecewisefJM(unlist(thetas))
    score.mat <- ScorePoissonMat.piecewisefJM(unlist(thetas)) 
  }  
  Hessian <- out$hessian
  lgLik <- out$value
